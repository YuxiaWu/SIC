...
def newCent_from_old(L, centroids_old):
    '''
    input: labeled dataset 
    output:cluster center with L
    '''
...
def _kmeans_single_elkan(X, y_labeled, sample_weight, centers_init, max_iter=300,
    ...
    # add two lines
    n_labeled_data = len(y_labeled)
    labels[:n_labeled_data] = y_labeled
    #print(n_clusters)
    #print(n_labeled_data)
    ...
    y_labeled_align = y_labeled.copy()
    for i in range(max_iter):
        ...
        labels[:n_labeled_data] = y_labeled_align
        #print('label done')
        #y_labeled_old = y_labeled_align.copy()
        #print('label_max',labels.max())
        #print('label_min',labels.min())
        centers_new_tmp = centers_new.copy()
        centers_new = newCent_from_old(np.hstack((X, labels.reshape(len(labels),1))), centers_new_tmp )
...
class Semi_KMeans(TransformerMixin, ClusterMixin, BaseEstimator):
    ...
    def _init_centroids_(self, X, x_squared_norms, n_cluster, init, random_state,
                        init_size=None):
    ...
    def fit_semi_kmeans(self, L, U, y, sample_weight=None):
        X = np.vstack((L,U))
        label_list = np.unique(y)
        k_label = len(label_list) 
        print('k_label: ', k_label)          #cluster number in L
        all_data_size = np.shape(X)[0]
        label_data_size = np.shape(L)[0]
        clusterAssment = np.zeros(all_data_size)
        clusterAssment[:label_data_size] = y
        # centroids of the labeled data                          
        centroids_labeled = newCent(np.hstack((L, y.reshape(len(y),1))))
        # centroids of the unlabeled data
        x_squared_norms_U = row_norms(U, squared=True)
        ...
        for i in range(self._n_init):
            ...
            centers_init = self._init_centroids(
                ...
                random_state=random_state)
            '''
            centroids_unlabeled = self._init_centroids_(
                U, x_squared_norms_U, n_cluster = 1,init=init,
                random_state=random_state)
                                
            centers_init = np.vstack((np.array(centroids_labeled), centroids_unlabeled))
            #print(centroids_labeled.dtype)
            del centroids_labeled, centroids_unlabeled
            #print(centroids_unlabeled.dtype)
            if self.verbose:
                print("Initialization complete")

            # run a k-means once
            labels, inertia, centers, n_iter_ = kmeans_single(
                X, y, sample_weight, centers_init, max_iter=self.max_iter,
...
'''
def semi_kMeans(L, U, k_all, distMeas=distEclud, initial_centriod=newCent):

    all_data = np.vstack((L[:,:-1],U))
    label_list = np.unique(L[:,-1])
    k_label = len(label_list)           #cluster number in L
    all_data_size = np.shape(all_data)[0]
    label_data_size = np.shape(L)[0]
    clusterAssment = np.zeros((all_data_size,1) )
    clusterAssment[:label_data_size,0] = L[:,-1]
    # centroids of the labeled data                          
    centroids_labeled = initial_centriod(L)
    # centroids of the unlabeled data
    x_squared_norms = row_norms(U, squared=True)
    centroids_unlabeled = KMeans(n_clusters = k_all - k_label)._init_centroids(U, x_squared_norms,init='k-means++', random_state=check_random_state(0))
